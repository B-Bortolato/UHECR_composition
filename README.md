...
This repository implements the method proposed in [this paper](https://arxiv.org/abs/2212.04760) to **infer the composition of ultra-high-energy cosmic rays (UHECRs)**. The method estimates the fraction of different primaries based on the longitudinal profile observables \(X_\text{max}\) and its uncertainty \(dX_\text{max}\).  

The final output is the **posterior distribution of possible compositions**:

\[
\mathbf{w} = (w_p, w_\text{He}, w_\text{Li}, \dots) \sim P(\mathbf{w} \mid \text{measured data})
\]
...
1. **Prepare simulated data**  
   ...
   Each primary array has the shape `(n_events, 3)`:
   ...
   ```
   - Energy is in [EeV]  
   - \(X_\text{max}\) and \(dX_\text{max}\) are in [g/cm²]
...
4. **Compute moments of \(X_\text{max}\)**  

   For each primary, compute the **first three non-central moments** \(z = (z_1, z_2, z_3)\) of \(X_\text{max}\) using the **bootstrap method** to account for statistical and systematic uncertainties.  
...
5. **Formulate the likelihood**  
   ...
   \[
   P_\text{sim}(z \mid \mathbf{w}) = \sum_Z P(z \mid \text{primary } Z) w_Z
   \]

   The posterior is obtained via Bayes’ theorem:

   \[
   P(\mathbf{w} \mid \text{measured data}) \propto L(\text{measured data} \mid \mathbf{w}) \cdot \text{Prior}(\mathbf{w})
   \]

   where the likelihood is defined as:

   \[
   L(\text{measured data} \mid \mathbf{w}) = \int \log P_\text{sim}(z \mid \mathbf{w}) \, P(z \mid \text{measured data}) \, dz
   \]
...
6. **Sample compositions**  
   ...
   Compositions \(\mathbf{w}\) are sampled using **Nested Sampling** via the [UltraNest](https://johannesbuchner.github.io/UltraNest/) package:

   \[
   \mathbf{w} \sim P(\mathbf{w} \mid \text{measured data})
   \]
